...
async def main(loop):
    with open("./src/config.yaml", encoding="utf8") as file:
        ...
        benchmark_messaging = BenchmarkMessaging(pulisher, subscriber, metric_repository)
        await benchmark_messaging.run(100)
        ...
        benchmark_api = BenchmarkAPI(client_api, metric_repository)
        benchmark_api.run(100)
